 ...
 * @param layerIndex
 * 		The index of the layer on which the tile whose image should be retrieve exists
...
/**
 * Get a property given to the map. Note that this method will not perform well and should not be used as part of the
...
/**
 * Get a propety given to a particular tile. Note that this method will not perform well and should not be used as part
 ...
 * @param lineByLine
 * 		True if we should render line by line, i.e. giving us a chance to render something else between lines (@see {@link
...
public String getObjectName(int groupID, int objectID) {
	if (groupID >= 0 && groupID < objectGroups.size()) {
		...
		if (objectID >= 0 && objectID < grp.objects.size()) {
			MapObject object = (MapObject) grp.objects.get(objectID);
			return object.name;
		}
	}
	return null;
