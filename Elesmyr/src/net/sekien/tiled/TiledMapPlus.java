...
/**
 * An extended version of the TiledMap class, with more functionality. A merge of Liam (liamzebedee) Edwards-Playne's
 * existing library, TiledMapPlus
 *
 * @author liamzebedee
 */
/*
 ...
 * 		The location where we can find the tileset images and other resources
 * @throws SlickException
 * 		Indicates a failure to load the tilemap
 */
...
/** Populates the objectGroupName to objectGroupOffset map and the groupObjectName to groupObjectOffset map */
...
/**
 * Writes the current TiledMap to a stream <br> Map is written using GZIP Base64 encoding in TiledMap V0.8
 *
 * @param stream
	...
	try {
		...
		for (int objectGroupI = 0; objectGroupI < this.objectGroups.size(); objectGroupI++) {
			...
			for (int groupObjectI = 0; groupObjectI < objectGroupData.objects
				...
				MapObject mapObject = objectGroupData.objects
						                      .get(groupObjectI);
				object.setAttribute("x", ""+mapObject.x);
				object.setAttribute("y", ""+mapObject.y);
				switch (mapObject.objectType) {
					case IMAGE:
						object.setAttribute("gid", ""+mapObject.gid);
						break;
					case RECTANGLE:
						object.setAttribute("name", mapObject.name);
						object.setAttribute("type", mapObject.type);
						object.setAttribute("width", ""+mapObject.width);
						object.setAttribute("height", ""+mapObject.height);
						break;
					case POLYGON:
						Element polygon = doc.createElement("polygon");
						String polygonPoints = "";
						for (int polygonPointIndex = 0; polygonPointIndex < mapObject.points.getPointCount()-1; polygonPointIndex++) {
							polygonPoints += mapObject.points.getPoint(polygonPointIndex)[0]+","+
									                 mapObject.points.getPoint(polygonPointIndex)[1]+" ";
						}
						polygonPoints.trim();
					...
					case POLYLINE:
						...
						for (int polyLinePointIndex = 0; polyLinePointIndex < mapObject.points.getPointCount()-1; polyLinePointIndex++) {
							polylinePoints += mapObject.points.getPoint(polyLinePointIndex)[0]+","+
									                  mapObject.points.getPoint(polyLinePointIndex)[1]+" ";
						}
						polylinePoints.trim();
				...
				if (mapObject.props!=null) {
					Element objectProps = doc.createElement("properties");
					Enumeration propertyEnum = mapObject.props
							                           .propertyNames();
					while (propertyEnum.hasMoreElements()) {
						String key = (String) propertyEnum.nextElement();
						...
						objectProperty.setAttribute("value",
								                           mapObject.props.getProperty(key));
