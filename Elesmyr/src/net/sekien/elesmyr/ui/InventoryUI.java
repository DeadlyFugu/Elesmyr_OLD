...
public class InventoryUI implements UserInterface {
	...
	public void update(GameContainer gc, GameClient receiver) {
		...
		InventoryEntry selitem = getItem(isel, receiver);
		...
		if (selitem != null) {
			if (in.isKeyPressed(Input.KEY_X)) {
				if (selitem.getItem().canEquip()) {
					MessageSystem.sendServer(null, new Message(ep.getReceiverName()+".equip", HBTTools.msgInt("i", inv.indexOf(selitem))), false);
				} else {
					MessageSystem.sendServer(null, new Message(ep.getReceiverName()+".use", HBTTools.msgInt("i", inv.indexOf(selitem))), false);
				}
			}
			for (int i = 1; i <= 5; i++)
				if (in.isKeyPressed(Input.KEY_1+i-1)) {
					MessageSystem.sendServer(null, new Message(ep.getReceiverName()+".slotn", new HBTCompound("p", new HBTTag[]{new HBTInt("n", i), new HBTInt("i", inv.indexOf(selitem))})), false);
				}
			if (in.isKeyPressed(Input.KEY_Z)) {
				MessageSystem.sendServer(null, new Message(ep.getReceiverName()+".drop", HBTTools.msgInt("i", inv.indexOf(selitem))), false);
			}
		}
		if (isel > smax-1)
			isel = smax-1;
