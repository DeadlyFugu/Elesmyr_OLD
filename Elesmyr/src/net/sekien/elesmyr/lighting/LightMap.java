...
public class LightMap {
	...
	public void update(Region r, Camera cam, float time) {
		...
		for (Light l : ((ArrayList<Light>) lights.clone())) {
			float xf = (float) ((jslx*gw+gw+l.x+(Math.floor((cam.getXOff()-jsx)/(float) gw)*gw))*xmul)-jsx;
			float yf = (float) ((jsly*gh+gh+l.y+(Math.floor((cam.getYOff()-jsy)/(float) gh)*gh))*ymul)-jsy;
