...
public class FileHandler {
	...
	public static void readData() throws IOException {
		...
		String pack_dir = Globals.get("packDir", "pack/");
		if (!pack_dir.endsWith("/")) {
			pack_dir = pack_dir.concat("/");
		}
		for (HBTTag tag : readHBTFile(pack_dir+"packs.hbtx")) {
				...
				for (HBTTag packentry : ((HBTCompound) tag)) {
					if (packentry instanceof HBTFlag) {
						if (((HBTFlag) packentry).isTrue()) {
							packs.add(pack_dir+packentry.getName());
						}
					}
				}
			}
