...
public class GameServer implements MessageEndPoint {
	...
	public void changePlayerRegion(String data, int x, int y, Connection connection, boolean killOld) {
		...
		ServerAudio.sendMusicUpdate(connection, world.getRegion(data));
	}
	...
	public Region getPlayerRegion(Connection connection) {
		return world.getRegion(playerEnt.get(players.get(connection)).split("\\.", 2)[0]);
	}
